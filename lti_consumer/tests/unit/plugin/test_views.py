...
class TestLti1p3KeysetEndpoint(TestCase):
    ...
    @ddt.data(
        ('lti_consumer:lti_consumer.public_keyset_endpoint', ['075194d3-6885-417e-a8a8-6c931e272f00']),
        ('lti_consumer:lti_consumer.public_keyset_endpoint_via_location', ['block-v1:x+x+x+type@problem+block@x']),
    ...
    @patch('lti_consumer.plugin.views.get_external_config_from_filter')
    def test_public_keyset_endpoint_using_external_id_in_url(self, get_external_config_from_filter):
        """
        Check that the endpoint is accessible using the external LTI configuration ID.
        """
        external_config = {'version': LtiConfiguration.LTI_1P3, 'lti_1p3_public_jwk': {}}
        get_external_config_from_filter.return_value = external_config

        response = self.client.get(reverse(
            'lti_consumer:lti_consumer.public_keyset_endpoint_via_external_id',
            args=['x', 'x']
        ))
        ...
        self.assertEqual(response['Content-type'], 'application/json')
        self.assertEqual(response['Content-Disposition'], 'attachment; filename=keyset.json')
        self.assertEqual(external_config["lti_1p3_public_jwk"], json.loads(response.content.decode('utf-8')))
...
class TestLti1p3AccessTokenEndpoint(TestCase):
    ...
    @patch('lti_consumer.plugin.views.get_lti_api_base')
    @patch('lti_consumer.plugin.views.LtiConsumer1p3')
    @patch('lti_consumer.plugin.views.get_external_config_from_filter')
    def test_access_token_endpoint_with_external_id_in_url(
        self,
        ...
        external_config = {
            'version': LtiConfiguration.LTI_1P3,
            'lti_1p3_client_id': 'client-id',
            'lti_1p3_private_key': 'private-key',
            'lti_1p3_private_key_id': 'private-key-id',
            'lti_1p3_tool_public_key': 'public-key',
            'lti_1p3_tool_keyset_url': 'keyset-url',
        }
        token = {'access_token': 'test-token'}
        get_external_config_from_filter.return_value = external_config
        lti_consumer.return_value.access_token.return_value = token
        ...
        get_external_config_from_filter.assert_called_once_with({}, 'x:x')
        get_lti_api_base.assert_called_once_with()
        lti_consumer.assert_called_once_with(
            iss=get_lti_api_base(),
            lti_oidc_url=None,
            lti_launch_url=None,
            client_id=external_config['lti_1p3_client_id'],
            deployment_id=None,
            rsa_key=external_config['lti_1p3_private_key'],
            rsa_key_id=external_config['lti_1p3_private_key_id'],
            redirect_uris=None,
    ...
    @ddt.data(
        ('lti_consumer:lti_consumer.access_token', ['075194d3-6885-417e-a8a8-6c931e272f00']),
        ('lti_consumer:lti_consumer.access_token_via_location', ['block-v1:x+x+x+type@problem+block@x']),
